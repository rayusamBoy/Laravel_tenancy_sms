<form class="ajax-update" action="{{ route('marks.update', [$exam->id, $my_class_id, $subject_id, $section_id ?? null]) }}" method="post">
    @csrf @method('put')
    <table class="table table-striped data-table marks">
        <thead>
        ...
        <tbody class="exam-tbody">
            ...
            <tr>
                ...
                <td class="text-center"><input title="EXAM" min="0" max="{{ $mk->exam->exam_denominator }}" class="text-center form-control form-control-sm" name="exm_{{ $mk->id }}" value="{{ $mk->exm }}" type="number"></td>
            </tr>
            @endforeach
        </tbody>
    </table>
